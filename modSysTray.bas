...
Private Type NOTIFYICONDATAA
   ...
   szInfo As String * 256     ' 416 (Text for balloon)
   uTimeOutOrVersion As Long  ' 420
   szInfoTitle As String * 64 ' 484 (Title text for balloon)
...
Dim lR As Long
   If (m_bSupportsNewVersion) Then
      ...
      Else
         nfIconDataA.szInfo = sMessage & vbNullChar
         nfIconDataA.szInfoTitle = sTitle & vbNullChar
...
Public Property Let ToolTip(ByVal sTip As String)
   If (m_bUseUnicode) Then
      stringToArray sTip, nfIconDataW.szTip, _
...
'Public Function AddMenuItem(ByVal sCaption As String, Optional ByVal sKey As _
...
Public Sub AddNotifyIcon(frm As Form, picIcon As Long, ByVal strTip As String)
   'Initialization: Get version
   Dim lMajor As Long
   Dim lMinor As Long
   Dim bIsNt As Long
   ...
   If (m_bUseUnicode) Then
      With nfIconDataW
         .hwnd = frm.hwnd
         .uID = vbNull
         .uFlags = NIF_ICON Or NIF_MESSAGE Or NIF_TIP 'Adding an icon, a tip and want a return message
         .uCallbackMessage = WM_MOUSEMOVE
         .hIcon = picIcon
         stringToArray strTip & " (dbl-click to open)", .szTip, _
                        unicodeSize(IIf(m_bSupportsNewVersion, 128, 64))
         If (m_bSupportsNewVersion) Then
   ...
   Else
      With nfIconDataA
         ...
         .uCallbackMessage = WM_MOUSEMOVE
         .hIcon = picIcon
         .szTip = strTip & " (dbl-click to open)" & Chr$(0)
         If (m_bSupportsNewVersion) Then
            .uTimeOutOrVersion = NOTIFYICON_VERSION
         End If
         .cbSize = nfStructureSize
...
Public Sub ModifyNotifyIcon(picIcon As Long, ByVal strTip As String)
    If (m_bUseUnicode) Then
        With nfIconDataW
            .hIcon = picIcon
            stringToArray strTip & " (dbl-click to open)", .szTip, _
    ...
    Else
        With nfIconDataA
            .hIcon = picIcon
            .szTip = strTip & " (dbl-click to open)" & vbNullChar
            .uFlags = NIF_ICON Or NIF_TIP
        End With
        Shell_NotifyIconA NIM_MODIFY, nfIconDataA
    End If
